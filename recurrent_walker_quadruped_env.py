...
class RecurrentWalkerEnv(QuadrupedEnv):
    def __init__(self):
        super().__init__()

    def _init_spaces(self):
        self.obs_length = 4 * 2 + 8 + 3 + 3 + 2 # 4 legs * 2 joint angles + 8 joints velocities + 3d base accel + 3d base gyro + (roll, pitch)
    ...
    def _compute_reward(self, action):
        ...
        return r_vx + r_z + r_homing_similarity + r_vz + r_orientation

    def _is_collapsed(self, state):
        roll, pitch = (state[-2], state[-1])
