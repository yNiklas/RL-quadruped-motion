...
env = make_vec_env(WalkerQuadrupedEnv, n_envs=16)
log_cb = LoggerCallback()

#model = PPO("MlpPolicy", env, device="cuda", verbose=1, tensorboard_log="./quadruped_tensorboard/", clip_range=0.1)
model = PPO.load("quadruped_ppo_model_gauss", env=env, device="cuda", verbose=1, tensorboard_log="./quadruped_tensorboard/")
